...
def update_front_end():
    # copying everything from build folder & putting it in a new folder called "chain-info"
    copy_folders_to_front_end("./build", "./front_end/src/chain-info")

    # sending the frontend of this project the config, but in json format
    with open("brownie-config.yaml", "r") as brownie_config:
        config_dict = yaml.load(
            brownie_config, Loader=yaml.FullLoader
        )  # loads the yaml to a dict
